...
class MainPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        inventory_btn = tk.Button(frame, padx = 24, image=self.inventory_btn_image, text="Access/Update" + "\n" + "Inventory", bg='#e8eb34', font= ('Courier', 15, 'bold'), command= lambda: controller.show_frame(InventoryPage))
        ...
        transaction_btn = tk.Button(frame, image= self.transaction_btn_image, text='Transaction Mode', bg='#e8eb34', command= lambda: controller.show_frame(transactionPage), font=('Courier', 15,'bold'))
        ...
        monthRevBtn = tk.Button(frame1, text='Show Analysis', bg= '#34a1eb', font= ('Courier', 14, 'bold'), command= lambda: showAnalysis())
...
class InventoryPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # adding 'see' button
        see_btn = tk.Button(top_frame, bg= '#e8eb34', text= 'SEE', font= ('Courier', 15, 'bold'), command= lambda: controller.show_frame(seeItemsPage))
        see_btn.place(relx= 0.02, rely= 0.25, relheight= 0.5, relwidth= 0.8/3)

        # adding 'add' button
        add_btn = tk.Button(top_frame, bg= '#e8eb34', text= 'ADD', font= ('Courier', 15, 'bold'), command= lambda: controller.show_frame(AddInventoryPage))
        add_btn.place(relx=0.8/3 + 0.1, rely=0.25, relheight=0.5, relwidth=0.8 / 3)

        # adding 'update' button
        update_btn = tk.Button(top_frame, bg= '#e8eb34', text='UPDATE', font= ('Courier', 15, 'bold'), command= lambda: controller.show_frame(updatePage))
        ...
        backBtn = tk.Button(backBtnFrame, font= ('Courier', 15, 'bold'), text= 'Back', command= lambda: controller.show_frame(MainPage))
...
class AddInventoryPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Add Item Button
        add_item_btn = tk.Button(frame, bg= '#e8eb34', font= ('Courier', 15, 'bold'), text= 'ADD', command= lambda: [add_item(name_entry.get(), int(barcode_entry.get()), float(pp_entry.get()), float(sp_entry.get()), int(qtn_entry.get())), erase_previous_entry(name_entry),
                                                                                                                     erase_previous_entry(barcode_entry), erase_previous_entry(sp_entry), erase_previous_entry(pp_entry), erase_previous_entry(qtn_entry)])
        add_item_btn.place(relx= 0.02, rely= 0.8, relheight= 0.1, relwidth= 0.1)

        # Add Back Button
        back_btn = tk.Button(frame, text= 'Back', font= ('Courier', 15, 'bold'), command= lambda: controller.show_frame(InventoryPage))
...
class updatePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # adding the buttons
        update_name_btn = tk.Button(updateOptionFrame, bg= '#e8eb34', font= ('Courier', 15, 'bold'),text= 'Update' + '\n'+ 'Name', command= lambda: controller.show_frame(updatingNamePage))
        update_name_btn.place(relx=0.01, rely=0.01, relheight=0.3, relwidth=0.3)

        update_pp_btn = tk.Button(updateOptionFrame, bg= '#e8eb34',font= ('Courier', 15, 'bold'), text= 'Update' + '\n' + 'P.Price', command= lambda: controller.show_frame(updatingPPPage))
        update_pp_btn.place(relx=0.69, rely=0.01, relheight=0.3, relwidth=0.3)

        update_sp_btn = tk.Button(updateOptionFrame, bg= '#e8eb34', font= ('Courier', 15, 'bold'),text= 'Update' + '\n'+ 'S.Price', command= lambda: controller.show_frame(updatingSPPage))
        update_sp_btn.place(relx=0.01, rely=0.69, relheight=0.3, relwidth=0.3)

        update_qtn_btn = tk.Button(updateOptionFrame, bg= '#e8eb34',font= ('Courier', 15, 'bold'),text= 'Update' + '\n'+ 'Quantity', command= lambda: controller.show_frame(updatingQtnPage))
        update_qtn_btn.place(relx=0.69, rely=0.69, relheight=0.3, relwidth=0.3)

        # Add Back Button
        back_btn = tk.Button(updateOptionFrame, text='Back',font= ('Courier', 15, 'bold'), command=lambda: controller.show_frame(InventoryPage))
...
class updatingNamePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        updateNameBtn = tk.Button(updateNameFrame, bg= '#e8eb34',font= ('Courier', 15, 'bold'), text= 'Update', command= lambda: update_name(enterNameEntry.get(), int(self.enterBarcodeEntry.get())))
        ...
        # Add Back Button
        back_btn = tk.Button(frame, text='Back',font= ('Courier', 15, 'bold'), command=lambda: controller.show_frame(updatePage))
...
class updatingPPPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        updatePPBtn = tk.Button(updatePPFrame, bg= '#e8eb34', font= ('Courier', 15, 'bold'),text= 'Update', command= lambda: update_pp(float(enterPPEntry.get()), int(enterBarcodeEntry.get())))
...
class updatingSPPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        updateSPBtn = tk.Button(updateSPFrame, bg= '#e8eb34', font= ('Courier', 15, 'bold'),text= 'Update', command= lambda: update_sp(float(enterSPEntry.get()), int(enterBarcodeEntry.get())))
...
class updatingQtnPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        updateQtnBtn = tk.Button(updateQtnFrame, bg= '#e8eb34',font= ('Courier', 15, 'bold'), text= 'Update', command= lambda: update_qnt(float(enterQtnEntry.get()), int(enterBarcodeEntry.get())))
...
class transactionPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Add a Enter button
        enter_btn = tk.Button(self.barcodeFrame, font= ('Courier', 15, 'bold'),bg= '#e8eb34', text= 'Enter', command= lambda: [add_item_to_transactions(item_database.getNameFromBarcode(int(self.barcodeEntry.get())),
                                                                                                     item_database.getSalePriceFromBarcode(int(self.barcodeEntry.get())),
                                                                                                     str(datetime.datetime.today()).split(' ')[0], item_database.getPPFromBarcode(int(self.barcodeEntry.get()))), erase_previous_entry(self.barcodeEntry),
        ...
        done_btn = tk.Button(self, text= 'Done', bg='#eb3434', font= ('Courier', 15, 'bold'),command= lambda: [done_btn_pressed(), controller.show_frame(MainPage)])
...
def main():
    ...
    inventoryDf.to_excel('Inventory.xlsx', index=False, sheet_name='Inventory')
